...
void track_sub_mat(Itc_Mat_t* src1, Itc_Mat_t* src2, Itc_Mat_t* dst)
{
...
#define MHT_TRACH_RESERVE_THRESHOLD 230  //只保留大于该值的点，否则置为0
#define MHT_TRACH_UPDATE_THRESHOLD 248  //大于该值的点，就不直接更新
void track_update_MHI(Itc_Mat_t* src1, Itc_Mat_t* src2, Itc_Mat_t* mhi, int diffThreshold, Itc_Mat_t* maskT, int Threshold)
{
	...
	int minusDiffThreshold = -diffThreshold;
	Track_Size_t sizeMat;
	...
	if (maskT == NULL)
	{
		for (i = 0; i < sizeMat.height; i++)
		{
			...
			{
				int k = (int)(qsrc1[j] - qsrc2[j]);
				if (k > diffThreshold || k<minusDiffThreshold)
				{
					qmhi[j] = qmhi[j]>MHT_TRACH_UPDATE_THRESHOLD ? qmhi[j] : 255;
				}
				else
				{
					//mhi不能取小于0的值
...
int track_intersect_rect(Track_Rect_t *rectA, Track_Rect_t *rectB, int expand_dis)
{
	...
	else
	{
		...
		if (expand_dis<0)
		{
			...
			int sizeArea = width_int*height_int;
...
int track_calculateDirect_ROI(Itc_Mat_t* mhi, Track_Rect_t roi, int *direct, float *dX, float *dY)
{
	...
	if (count_changeX > (threshold>>1))
		gradientH = sum_gradientH / (count_changeX*k_int_enhance);
	if (count_changeY > (threshold>>1))
...
int track_copyImage(Itc_Mat_t* src, Itc_Mat_t* dst)
{
	...
	if (!ITC_ARE_TYPES_EQ(src, dst))		//检测类型是否一致
	{	
		ITC_ERROR_("矩阵类型不一致");
		return FALSE;
	}

	if (!ITC_ARE_SIZES_EQ(src, dst))		//检查大小是否一致
	{	
		ITC_ERROR_("矩阵大小不一致");
		return FALSE;
	}

	int type = ITC_MAT_TYPE(src->type);		//类型
	int depth = ITC_MAT_DEPTH(type);		//深度
	int channel = ITC_MAT_CN(type);			//通道数
	...
	int i = 0;
	itc_uchar* src_p = src->data.ptr;
	itc_uchar* dst_p = dst->data.ptr;
	int dst_step = dst->width;
	int src_step = src->width;
	for (i = 0; i < src->height;i++)
	{
		memcpy(dst_p, src_p, sizeof(itc_uchar)* dst_step);
		dst_p += dst_step;
		src_p += src_step;
	}
	return TRUE;
...
int track_sub_matROI(Itc_Mat_t* src1, Itc_Mat_t* src2, Track_Rect_t roi)
{
	...
	if (!ITC_ARE_TYPES_EQ(src1, src2))		//检测类型是否一致
	{
		ITC_ERROR_("矩阵类型不一致");
		return FALSE;
	}

	if (!ITC_ARE_SIZES_EQ(src1, src2))		//检查大小是否一致
	{
		ITC_ERROR_("矩阵大小不一致");
		return FALSE;
	}
	...
	int i = 0;
	int j = 0;
	int src1_step = src1->width;
	int src2_step = src2->width;
	itc_uchar* src1_pStar = src1->data.ptr + src1_step*roi.y + roi.x;
	itc_uchar* src2_pStar = src2->data.ptr + src2_step*roi.y + roi.x;
	for (i = 0; i < roi.height; i++)
	{
		itc_uchar* src1_p = src1_pStar;
		itc_uchar* src2_p = src2_pStar;
		for (j = 0; j < roi.width; j++)
		{
			tempSub = src1_p[j] - src2_p[j];
			if (tempSub>5 || tempSub<-5)
			{
				count++;
			}
		}
		src1_pStar += src1_step;
		src2_pStar += src2_step;
	}
	return count;
}

#define COEFF_STUTRACK_HISTOGRAM_DIMEN 16
#define COEFF_STUTRACK_HISTOGRAM_DIMEN_LOG2 4  //=log(COEFF_STUTRACK_HISTOGRAM_DIMEN)
double track_histogramContrast_matROI(Itc_Mat_t* src1, Itc_Mat_t* src2, Track_Rect_t roi)
{
	if (src1 == NULL || src2 == NULL)
	{
		return FALSE;
	...
	int i = 0;
	int j = 0;
	int sizeImg = roi.height*roi.width;
	int histogram_src1[COEFF_STUTRACK_HISTOGRAM_DIMEN] = { 0 };
	int histogram_src2[COEFF_STUTRACK_HISTOGRAM_DIMEN] = { 0 };
	int renge = COEFF_STUTRACK_HISTOGRAM_DIMEN_LOG2;
	int src1_step = src1->width;
	int src2_step = src2->width;
	itc_uchar* src1_pStar = src1->data.ptr + src1_step*roi.y + roi.x;
	itc_uchar* src2_pStar = src2->data.ptr + src2_step*roi.y + roi.x;
	...
	for (i = 0; i < roi.height; i++)
	{
		for (j = 0; j < roi.width; j++)
		{
			histogram_src1[(src1_p[j] >> renge)]++;
			histogram_src2[(src2_p[j] >> renge)]++;
		}
		src1_pStar += src1_step;
		src2_pStar += src2_step;
		src1_p = src1_pStar;
	...
	int disSum = 0;
	int diff = 0;
	for (i = 0; i < COEFF_STUTRACK_HISTOGRAM_DIMEN; i++)
	{
		diff = histogram_src1[i] - histogram_src2[i];
		disSum += diff*diff;
	}

	return sqrt(((double)disSum) / (sizeImg*sizeImg));
}
